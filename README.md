# Waiting for DK · v5 Multi-page Explore

这版已经重构成多页探索网站：

- index.html — 首页：倒数、进度、主视觉、六个探索入口
- explore.html — 情绪入口、Surprise Me、DK Universe
- calendar.html — 完整 D-Day 月历
- archive.html — 搜索与筛选
- timeline.html — Era Timeline + On This Day
- now.html — Weverse / Instagram / X 官方入口

共享文件：
- assets/style.css
- assets/app.js
- assets/data.js

这版使用 data.js，所以直接双击 HTML 也能运行大部分互动；上传 GitHub Pages 后页面之间会正常跳转。

- covers.html — DK Cover Archive（第一批 9 首官方 Cover）

- v7: covers.html upgraded to a custom YouTube player using the user's DK player artwork, with random initial track, playlist switching, favorite, previous/next, play/pause, shuffle, repeat, seek bar.

- v8: Music Room categories: Covers / Solo / OST / Special, mixed official YouTube + Spotify embeds.
